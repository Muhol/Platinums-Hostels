...
import { Wifi, Bed, Monitor, Lock, Coffee, ShowerHead, ShieldCheck, Zap, Home } from 'lucide-react'
...
export default function RoomDetailsContent({ room, similarRooms }: RoomDetailsContentProps) {
    const [slide, setSlide] = useState(false);
    const [activeImage, setActiveImage] = useState(room.images && room.images.length > 0 ? room.images[0] : room.image);
                                        ...
                                        <div>
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                {room.features.map((feature, idx) => {
                                                    const Icon = AMENITY_ICONS[feature] || Home;
                                                    return (
                        ...
                        <div className="flex flex-col gap-6">
                            ...
                            <div className="flex flex-col gap-4">
                                {similarRooms.length > 0 ? similarRooms.map((similar) => (
                                    <Link href={`/rooms/${similar.id}`} key={similar.id} className="flex gap-4 group cursor-pointer bg-white border border-gray-100 rounded-lg p-3 hover:shadow-md transition-all duration-300">
